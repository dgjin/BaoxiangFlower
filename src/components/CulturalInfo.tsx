...
export default function CulturalInfo({ isMobile = false }: CulturalInfoProps) {
  ...
  return (
    <div className={`bg-[#09090b]/90 backdrop-blur-xl border border-[#D4AF37]/25 rounded-2xl shadow-2xl text-gray-200 ${isMobile ? 'p-4 rounded-[20px]' : 'p-6'}`}>
      <div className="flex items-center gap-2 text-[#D4AF37] font-medium mb-4 font-serif">
        <BookOpen className="w-5 h-5 animate-pulse" />
        <span className={`tracking-wider font-semibold ${isMobile ? 'text-[15px]' : 'text-base'}`}>宝相华美学小传</span>
      </div>

      {/* Tab Navigation - iOS Segmented Style on Mobile */}
      {isMobile ? (
      ...
      {/* Content Canvas */}
      <div className={`relative overflow-hidden ${isMobile ? 'mt-2' : 'mt-4 min-h-[220px]'}`}>
        <AnimatePresence mode="wait">
          ...
          >
            <div>
              <h3 className={`font-bold text-gray-100 flex items-center gap-2 font-serif ${isMobile ? 'text-[15px]' : 'text-base'}`}>
            ...
            <div className={`text-xs text-gray-300 leading-relaxed font-sans ${isMobile ? 'space-y-2.5' : 'space-y-3'}`}>
